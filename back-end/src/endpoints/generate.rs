...
async fn generate(body: Json<Body>) -> Result<HttpResponse, Error> {
    ...
    for (endpoint_name, stuff) in endpoints_props {
        let file_name = endpoint_name.snake_to_kebab_case();

        let rendered = readonly_endpoint::render(&endpoint_name, stuff.inputs, stuff.outputs);

        zip_writer
            .start_file(format!("/endpoints/{}.ts", &file_name), zip_options)
            .unwrap();
        zip_writer.write_all(rendered.as_bytes()).unwrap();
    }
